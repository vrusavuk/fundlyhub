-- Create 1000 sample fundraisers with unique timestamps
DO $$
DECLARE
    i INTEGER;
...
BEGIN
    ...
    RAISE NOTICE 'Creating 1000 sample fundraisers using user ID: %', existing_user_id;
    ...
    FOR i IN 1..1000 LOOP
        ...
        current_timestamp_ms := (extract(epoch from clock_timestamp()) * 1000)::BIGINT;
        random_slug := lower(regexp_replace(title_base, '[^a-zA-Z0-9\s]', '', 'g'));
        random_slug := lower(replace(random_slug, ' ', '-')) || '-' || current_timestamp_ms || '-' || i;
        ...
        random_story := '<p>We are reaching out for your support during this challenging time. Your donation can make a real difference in our community.</p>' ||
        ...
        INSERT INTO public.fundraisers (
            ...
            random_created_at,
            random_created_at
        ) RETURNING id INTO fundraiser_id;

        -- Add some random donations for some fundraisers
        IF random() < 0.7 THEN -- 70% of fundraisers get donations
            donation_count := (1 + random() * 15)::INTEGER; -- 1-15 donations
    ...
    RAISE NOTICE 'Successfully created 1000 sample fundraisers with donations!';
    
    -- Show summary
    DECLARE
        total_fundraisers INTEGER;
        total_donations INTEGER;
        total_raised NUMERIC;
    BEGIN
        SELECT COUNT(*) INTO total_fundraisers FROM public.fundraisers;
        SELECT COUNT(*) INTO total_donations FROM public.donations;
        SELECT COALESCE(SUM(amount), 0) INTO total_raised FROM public.donations WHERE payment_status = 'paid';
        
        RAISE NOTICE 'Database now contains:';
        RAISE NOTICE '- % total fundraisers', total_fundraisers;
        RAISE NOTICE '- % total donations', total_donations;
        RAISE NOTICE '- $% total raised', total_raised;
    END;
END $$;