-- Fix campaign organizers by distributing them across different users
...
-- Add organization members based on campaign ownership
...
-- Update campaign titles and descriptions to match the new organizers
UPDATE public.fundraisers 
SET 
  title = CASE 
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Sarah Johnson') 
      THEN 'Emergency Medical Fund for Sarah'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Michael Chen') 
      THEN 'Community Garden Project'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Emma Rodriguez') 
      THEN 'School Library Books Drive'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'David Kim') 
      THEN 'Ocean Cleanup Campaign'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Lisa Thompson') 
      THEN 'Youth Sports Equipment Fund'
    ELSE title
  END,
  beneficiary_name = CASE 
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Sarah Johnson') 
      THEN 'Sarah Johnson Medical Fund'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Michael Chen') 
      THEN 'Green Community Initiative'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Emma Rodriguez') 
      THEN 'Lincoln Elementary School'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'David Kim') 
      THEN 'Ocean Protection Alliance'
    WHEN owner_user_id = (SELECT id FROM public.profiles WHERE name = 'Lisa Thompson') 
      THEN 'Local Youth Sports League'
    ELSE beneficiary_name
  END;

-- Clear existing subscriptions and create new ones with the diversified users
